...
app.get('/health', (_, res) => res.json({ ok: true, status: botState.status }));
...
io.on('connection', (socket) => {
  ...
  socket.on('setMode', (mode) => {
    ...
    if (!['FARMING','PATROL','SOCIAL'].includes(mode)) return;
...
module.exports = {
  ...
  start(port) {
    httpServer.listen(port, '0.0.0.0', () => {
      console.log(`[Panel] Servidor web corriendo en puerto ${port}`);
    });
  },
};
