...
let patrolInterval = null, active = false, points = [], idx = 0;
...
module.exports = {
  ...
  startPatrol(bot, onAlert) {
    ...
    points = Array.from({ length: 6 }, (_, i) => {
      ...
      return { x: Math.round(c.x + Math.cos(a) * 18), y: c.y, z: Math.round(c.z + Math.sin(a) * 18) };
    });
    idx = 0;
    ...
    patrolInterval = setInterval(() => {
      ...
      if (near.length > 0 && onAlert)
        onAlert(`👤 Jugador detectado: ${near.map(p => p.username || '?').join(', ')}`);

      const pt = points[idx];
      try { bot.pathfinder.setGoal(new GoalNear(pt.x, pt.y, pt.z, 3), true); } catch (_) {}
      if (bot.entity.position.distanceTo({ x: pt.x, y: pt.y, z: pt.z }) < 5)
